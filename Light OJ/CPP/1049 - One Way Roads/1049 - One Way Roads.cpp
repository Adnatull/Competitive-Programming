...
int dfs(int source, int parent, vector < node > graph[]) { 
		if(source==0) return 0;
		rep(i,(int)graph[source].size()) { 
				int v = graph[source][i].v;
				if(v!=parent) return dfs(v,source,graph)+graph[source][i].w;
		}
		return 0;
}
...
int main() { 
	...
	test_case(t) { 
		...
		print_case;
		cout << min(dfs(graph[0][0].v, 0,graph)+graph[0][0].w, dfs(graph[0][1].v,0,graph)+graph[0][1].w) << endl;
	}
	return 0;
}
